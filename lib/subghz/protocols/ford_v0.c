...
static bool ford_v0_process_data(SubGhzProtocolDecoderFordV0* instance) {
    ...
    if(instance->bit_count == 80) {
        ...
        decode_ford_v0(
            instance->key1, instance->key2, &instance->serial, &instance->button, &instance->count);
        instance->key2 = key2;
...
SubGhzProtocolStatus
    ...
    decode_ford_v0(
        instance->generic.data, key2_old, &instance->serial, &instance->button, &instance->count);
    ...
    uint8_t key_data[8];
    for(int i = 0; i < 8; i++)
        key_data[7 - i] = (instance->key1 >> (i * 8)) & 0xFF;
